...
(defn- run-handlers
  "Called by realize-handlers to look at one specific element or attribute map. If item is an
  attribute map, runs any function values in the map, passing in fake-event as the sole argument.
  See docstring for realize-handlers function."
  [item]
  (if-not (map? item)
    item
    (reduce (fn [m k]
...
(defn realize-handlers
  ...
  [elem]
  (w/prewalk (fn [e] (run-handlers e)) elem))
